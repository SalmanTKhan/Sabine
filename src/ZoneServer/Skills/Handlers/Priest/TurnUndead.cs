...
namespace Sabine.Zone.Skills.Handlers.Priest
{
	...
	public class TurnUndeadHandler : ITargetedSkillHandler
	{
		...
		public void Handle(UseSkillParams parameters)
		{
			...
			var killChance = 20 + 2 * (caster.Parameters.BaseLevel - target.Parameters.BaseLevel) + 2 * level;
			killChance = System.Math.Clamp(killChance, 0, 100);

			if (rnd.Next(100) < killChance)
			{
				target.TakeDamage(target.Parameters.Hp, caster);
				Send.ZC_NOTIFY_SKILL(caster, target.Handle, skill.Id, level, target.Parameters.Hp, 0, 1, ActionType.Skill);
				return;
			}
